...
func messageToQueryParameters(message *descriptor.Message, reg *SRegistry, pathParams []descriptor.Parameter) (params []swaggerParameterObject, err error) {
...
func queryParams(message *descriptor.Message, field *descriptor.Field, prefix string, reg *SRegistry, pathParams []descriptor.Parameter) (params []swaggerParameterObject, err error) {
...
func findServicesMessagesAndEnumerations(s []*descriptor.Service, reg *SRegistry, m messageMap, ms messageMap, e enumMap, refs refMap) {
	for _, svc := range s {
...
func findNestedMessagesAndEnumerations(message *descriptor.Message, reg *SRegistry, m messageMap, e enumMap) {
...
// 根据字段的注释，更新字段含义等 按照规则解析出配置，如 `required` `pattern=\w\d]{15}` `enum:1,2` 真正注释
func updateFieldData(data *swaggerSchemaObject) {
	desc := data.Description
	reg,_ := regexp.Compile("`.*?`")
	arr := reg.FindAllString(desc, 100)

	if len(arr) != 0 {
		li := strings.LastIndex(desc, "`")
		if li != -1 {
			newDesc := strings.TrimSpace(desc[li+1:])
...
func renderMessagesAsDefinition(messages messageMap, d swaggerDefinitionsObject, reg *SRegistry, customRefs refMap) {
	for name, msg := range messages {
		...
		for _, f := range msg.Fields {
			...
			if fieldValue.Required != nil {
				schema.Required = append(schema.Required, kv.Key)
			}
...
func renderMessagesAsStreamDefinition(messages messageMap, d swaggerDefinitionsObject, reg *SRegistry) {
	for name, msg := range messages {
		if skipRenderingRef(name) {
			continue
		}
...
func schemaOfField(f *descriptor.Field, reg *SRegistry, refs refMap) swaggerSchemaObject {
	const (
		singular = 0
		array    = 1
		object   = 2
...
func renderEnumerationsAsDefinition(enums enumMap, d swaggerDefinitionsObject, reg *SRegistry) {
...
func fullyQualifiedNameToSwaggerName(fqn string, reg *SRegistry) string {
...
var registriesSeen = map[*SRegistry]map[string]string{}
...
// 获得 service 在真正文件中的具体 index，用于获取注释
func getServiceIndex(svc *descriptor.Service) int32 {
	for index, item := range svc.File.GetService() {
		if item == svc.ServiceDescriptorProto {
			return int32(index)
		}
	}
	return 0
...
func renderServices(services []*descriptor.Service, paths swaggerPathsObject, reg *SRegistry, requestResponseRefs, customRefs refMap) error {
	// Correctness of svcIdx and methIdx depends on 'services' containing the services in the same order as the 'file.Service' array.
	for _, svc := range services {
		svcIdx := getServiceIndex(svc)

		svcComments := protoComments(reg, svc.File, nil, "Service", serviceProtoPath, svcIdx)
		...
		for methIdx, meth := range svc.Methods {
			for bIdx, b := range meth.Bindings {
				...
				if b.ResponseBody == nil || len(b.ResponseBody.FieldPath) == 0 {
					...
					if !isWkn {
						if reg.wrapRespCode {
							responseSchema.schemaCore = schemaCore{
								Type:    "object",
							}
							ps := swaggerSchemaObjectProperties{}
							ps = append(ps, keyVal{
								Key:   "code",
...
func fieldProtoComments(reg *SRegistry, msg *descriptor.Message, field *descriptor.Field) string {
...
func enumValueProtoComments(reg *SRegistry, enum *descriptor.Enum) string {
...
func protoComments(reg *SRegistry, file *descriptor.File, outers []string, typeName string, typeIndex int32, fieldPaths ...int32) string {
	...
	for _, loc := range file.SourceCodeInfo.Location {
		...
		// add by peter 兼容字段的 trailingComments
		if loc.TrailingComments != nil {
			tComments = strings.TrimRight(*loc.TrailingComments, "\n")
			tComments = strings.TrimSpace(tComments)
			tComments = strings.Replace(tComments, "\n ", "\n", -1)
		}
		comments = comments + tComments
		return comments
	}
	return ""
}
...
func protoJSONSchemaToSwaggerSchemaCore(j *swagger_options.JSONSchema, reg *SRegistry, refs refMap) schemaCore {
	ret := schemaCore{}
...
func swaggerSchemaFromProtoSchema(s *swagger_options.Schema, reg *SRegistry, refs refMap) swaggerSchemaObject {
	ret := swaggerSchemaObject{
...
func addCustomRefs(d swaggerDefinitionsObject, reg *SRegistry, refs refMap) {
	if len(refs) == 0 {
		return
	}
